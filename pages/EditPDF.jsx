...
export default function EditPDF() {
    ...
    pdfDoc
      ...
      .then((page) => {
        ...
        const availableWidth =
          Math.min(stageRef.current?.clientWidth || 820, 860) - 24;
        const scale =
          Math.max(0.6, Math.min(1.6, availableWidth / baseViewport.width)) *
          zoom;
        ...
        return Promise.all([
          renderPromise,
          page.getTextContent(),
          Promise.resolve({ viewport, scale, pageHeight: baseViewport.height }),
          ...
          const boxes = textContent.items
            .map((item, index) => {
              ...
              const width = Math.max(
                (item.width || text.length * 6) * metrics.scale,
                8,
              );
        ...
        current.map((layer) => {
          ...
          if (interaction.mode === "resize") {
            return {
              ...
              y: Math.max(
                0,
                Math.min(pageSize.height - 18, interaction.startY - dy),
              ),
            };
          }
          ...
          return {
            ...
            x: Math.max(
              0,
              Math.min(pageSize.width - layer.width, interaction.startX + dx),
            ),
            y: Math.max(
              0,
              Math.min(pageSize.height - layer.height, interaction.startY - dy),
            ),
          };
        }),
      );
  ...
  function handlePdfDrop(event) {
    ...
    setSelectedPdf(
      Array.from(event.dataTransfer.files || []).find(
        (nextFile) => nextFile.type === "application/pdf",
      ),
    );
  }
  ...
  function getPointFromEvent(event, width = 180, height = 56) {
    ...
    const y =
      pageSize.height - (event.clientY - rect.top) / pageSize.scale - height;
    return {
  ...
  function addLayerAt(event) {
    ...
    const point = getPointFromEvent(
      event,
      activeTool === "text" ? 220 : 170,
      activeTool === "text" ? 34 : 56,
    );
  ...
  function addImage(event) {
    ...
    image.onload = () => {
      setImagePreviews((current) => [
        ...current,
        { url, ratio: image.width / image.height },
      ]);
    };
    image.src = url;
  ...
  function replaceDetectedText(box) {
    ...
    const replacement = createLayer(
      ...
      pageNumber,
      box.x,
      box.y + Math.max(1, box.pdfHeight * 0.12),
      {
      ...
      <div className="animate-fade-in">
        <section className="mx-auto max-w-4xl space-y-6 py-4 text-center sm:py-8">
          <div className="space-y-3">
            ...
            >
              Add text, images, highlights, whiteout blocks, or click detected
              PDF text to replace it, then export the finished document.
            </p>
          </div>
          ...
          {error ? (
            <div
              className="rounded-xl px-4 py-3 text-sm flex items-start gap-2 text-left"
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div className="space-y-2">
          ...
          <p
            ...
            style={{ color: "var(--text-muted)" }}
          >
            Add text, images, highlights, whiteout blocks, or click detected PDF
            text to replace it, then export the finished document.
          </p>
        </div>
        <button
      ...
      <div className="grid xl:grid-cols-[260px_1fr_300px] gap-5 items-start">
              ...
              <button
                ...
                className={
                  activeTool === item.tool
                    ? "btn-primary justify-center px-3"
                    : "btn-secondary justify-center px-3"
                }
                onClick={() => {
        ...
        <main
          ref={stageRef}
          className="min-h-[520px] overflow-auto rounded-lg border p-3"
          style={{
        ...
        >
          {!file ? (
            <div
              className="min-h-[500px] flex flex-col items-center justify-center text-center gap-3"
              style={{ color: "var(--text-muted)" }}
            >
              <FileImage size={34} />
              <div>
                ...
                <p className="text-sm mt-1">
                  Then add text, images, shapes, or whiteout blocks on the page.
                </p>
              </div>
            </div>
          ) : (
            <div
              ...
              style={{
                width: pageSize.viewWidth || 1,
                height: pageSize.viewHeight || 1,
                background: "#fff",
              }}
              onClick={addLayerAt}
            >
              <canvas ref={canvasRef} className="block" />
              {showDetectedText || activeTool === "edit-text"
                ? textBoxes.map((box) => (
                    <button
                      key={box.id}
                      type="button"
              ...
              {visibleLayers.map((layer) => {
                ...
                const top =
                  (pageSize.height - layer.y - layer.height) * pageSize.scale;
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderColor: isSelected
                        ...
                        : "rgba(37,99,235,0.45)",
                      background:
                        layer.type === "rect" || layer.type === "whiteout"
                          ? layer.fillColor
                          : "transparent",
                      opacity: layer.opacity,
                      cursor: "move",
          ...
          <div className="card p-4 space-y-4">
            ...
            {selectedLayer ? (
                ...
                {selectedLayer.type === "text" ? (
                  <>
                    <textarea
                      ...
                      onChange={(event) =>
                        updateLayer(selectedId, { text: event.target.value })
                      }
                    />
                    <div className="grid grid-cols-2 gap-2">
                      <input
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          updateLayer(selectedId, { bold: !selectedLayer.bold })
                        }
                      >
                        Bold
                      </button>
              ...
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>
                Select a layer on the page to edit its text, size, color, or
                opacity.
              </p>
            )}
          </div>
